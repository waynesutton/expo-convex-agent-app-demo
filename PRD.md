...
- Soft grey ground `#FAFAFA`, mist `#F2F2F2` for raised surfaces, field `#EAEAEA` for inputs
- Ink `#171717` for primary text, the user's bubble, and primary actions
- Slate `#4D4D4D` for secondary text, faint `#888888` for metadata
- Signal red `#EE0000` for errors only
- Bot identity colors from `BOT_COLORS` (greys plus blue, teal, amber, coral; no purple), one per bot, used for avatars and attribution
- Type: system sans for UI, monospace for the wordmark, tool calls, timestamps, and anything machine generated. The wordmark is weight 400, names and titles 500, buttons cap at 600; no display sizes.
